...
print(f"The result of {eq_literal} is: {eq_result}.")
...
## every 4 years from 1700 to 2500
leap_years = set(range(1700, 2500, 4))
# remove all years that are divisible by 100 and not divisible by 400.
"""two equivalent ways of going through all numbers in a set or list"""
# list comprehension: make a list with a for loop and an if statement
ly = set([y for y in leap_years if y%100 == 0 and y%400 != 0])
# Explicit for loop and if statements.
ly = set()
for y in leap_years:
    if y % 100 == 0 and y % 400 != 0:
        ly.add(y)
# remove the years that are not leap years
leap_years = leap_years.difference(ly)
# number of days in each month.  This variable is a Tuple, which is like
# an array but it cannot be changed.
...
# make input strings into two lists with [day, month, year]
numdate1 = [int(i) for i in date1.split()]
numdate2 = [int(i) for i in date2.split()]
...
if first_date[2] != second_date[2]:
    ...
    # x-(x-a) - (x-b) == a + b - x,
    # where, x = 356, a second day to start of the year,  b first day to end of the year
    how_many_days = second_to_startofyear + first_to_endofyear - 365

# how many years and months do  not depend on leap years.
how_many_years = second_date[2] - first_date[2]
how_many_months = second_date[1] + 12-first_date[1] + int(len(years_btw))*12