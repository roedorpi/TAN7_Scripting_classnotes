...
##
cv = CountVectorizer()
# transform the data, the list of sentences becomes a matrix where each row represent sentences and
# each column represents every word in the data. The value in each cell represents the number
# of times a given word appears in that sentence. Parameters of the vectorization can be done in the
# initialization of the vectorizer... see the documentation.
...
for i,k in enumerate(ngrams2_s):
...
for i,k in enumerate(ngrams2_ns):
...
for i,k in enumerate(ngrams3_s):
...
for i,k in enumerate(ngrams3_ns):
