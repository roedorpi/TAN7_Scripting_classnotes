...
# files can be saved with different encodings, that is different ways to map symbols to ascii code, you have probably experienced this when opening a file from a different language or operating system, where special characters are shown wrong. The open function alows you to specify the encoding of the file.
...
"""
## CSV files
a csv file (comma separated values file) is a plain text file that uses specific structure to arrange tabular data. Meaning: each piece of data is separated by a delimiter (a comma, a tab, a colon, or a semi-colon), and then when we parse the file, we read the delimiter as a character separating data (often into tables). 
...
with open('birthdays.csv', mode='w', newline="") as my_csv:
...
with open('fruit.csv', mode='w', newline='') as csv_file:
    fieldnames = ['fruit', 'color', 'taste']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)

    writer.writeheader()
    writer.writerow({'fruit': 'banana', 'color': 'yellow', 'taste': 'delicious'})
    writer.writerow({'fruit': 'apple', 'color': 'red', 'taste': 'meh'})
